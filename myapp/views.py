...
def view_profile_student(request):
    ...
    date = Notification.objects.filter().order_by('-date')
    d = ''
    if date.exists():
        l = date[0]
...
def view_notification(request):
    ...
    notifyclub= Notification_club.objects.filter(CLUB__club_members__LOGIN_id=lid,CLUB__club_members__status='approved' ).order_by('-date', '-id')
    notifyoffice= Notification_office_staff.objects.all().order_by('-date', '-id')
    notifyteacher= Notification_teacher.objects.filter(TEACHERS__sujectallocation__SUBJECT__COURSE=Student.objects.get(LOGIN_id=lid).COURSE).order_by('-date', '-id')
    print(notifyteacher)
    ...
    for i in notifyclub:
        l.append({'id':i.id,'notification':i.notification+' \nfrom: '+i.CLUB.name,'date':i.date})
    for i in notifyoffice:
        l.append({'id':i.id,'notification':i.notification+' \nfrom: '+i.OFFICE_STAFF.office_staff_name,'date':i.date})
    nt = []
    for i in notifyteacher:
        if i.id in nt:
            continue
        nt.append(i.id)
        l.append({'id':i.id,'notification':i.notification+' \nfrom: '+i.TEACHERS.name,'date':i.date})
    print(l)
...
def view_push_notification(request):
    ...
    try:
        ...
        lid = request.POST["lid"]
        notify = Notification.objects.all().order_by('id')
        notifyclub = Notification_club.objects.filter(CLUB__club_members__LOGIN_id=lid,CLUB__club_members__status='approved').order_by('id')
        notifyoffice = Notification_office_staff.objects.all().order_by('id')
        notifyteacher = Notification_teacher.objects.filter(
            TEACHERS__sujectallocation__SUBJECT__COURSE=Student.objects.get(LOGIN_id=lid).COURSE).order_by('id'
                                                                                                )
        print(notifyteacher)
        # notifyteacher= Notification_teacher.objects.filter(TEACHERS__LOGIN__student__LOGIN_id=lid).order_by('-date', '-id')
        l = []
        index_counter = 0
        for i in notify:
            l.append({'id': i.id, 'notification': i.notification, 'date': i.date, 'nid': index_counter})
            index_counter += 1
        for i in notifyclub:
            l.append({'id': i.id, 'notification': i.notification + ' \nfrom: ' + i.CLUB.name, 'date': i.date,
                      'nid': index_counter})
            index_counter += 1
        for i in notifyoffice:
            l.append({'id': i.id, 'notification': i.notification + ' \nfrom: ' + i.OFFICE_STAFF.office_staff_name,
                      'date': i.date, 'nid': index_counter})
            index_counter += 1
        nt = []
        for i in notifyteacher:
            if i.id in nt:
            ...
            index_counter += 1
        print(l)
        # l.sort(key=lambda x: (x['date']), reverse=False)
        l.sort(key=lambda x: (x['date'], x['nid'] > int(nid)), reverse=True)
        print(nid)
        print(l)
        if len(l) > 0:
            i = l[0]
            ...
            print(i['nid'] > int(nid))
            return JsonResponse({"status": "ok", 'nid': i['nid'], "message": i['notification']})
        else:
            return JsonResponse({"status": "no"})
    except:
        return JsonResponse({"status": "no"})
